...
#include "../../../DiligentSamples/ThirdParty/glfw/include/GLFW/glfw3native.h"
#ifdef GetObject
#    undef GetObject
#endif
#ifdef CreateWindow
...
bool AppBase::CreateWindow(const char* Title, int Width, int Height, int GlfwApiHint)
{
...
bool AppBase::InitEngine(RENDER_DEVICE_TYPE DevType)
{
#if PLATFORM_WIN32
...
void AppBase::GLFW_ResizeCallback(GLFWwindow* wnd, int w, int h)
{
...
void AppBase::GLFW_KeyCallback(GLFWwindow* wnd, int key, int, int state, int)
{
    auto* pSelf = static_cast<AppBase*>(glfwGetWindowUserPointer(wnd));
...
void AppBase::GLFW_MouseWheelCallback(GLFWwindow* wnd, double dx, double dy)
{
}
...
void AppBase::OnKeyEvent(Key key, KeyState newState)
{
...
bool AppBase::ProcessCommandLine(int argc, const char* const* argv, RENDER_DEVICE_TYPE& DevType)
{
...
{
    std::unique_ptr<AppBase> Samp{CreateApp()};
